...
| `MESH_NON_MANIFOLD` | info | Edges shared by three or more faces. Harmless for display; breaks 3D printing, booleans and some simplifiers. | Repair in a DCC if printing or physics matter; optimize_glb tolerates it. |
| `MESH_DEGENERATE_FACES` | info | Zero-area or repeated-corner faces. | optimize_glb prunes them. |
| `NORMALS_MISSING` | warning | No authored normals; viewers compute their own (smooth or flat depending on the viewer), so shading differs between apps. | optimize_glb writes smooth normals; export_usdz generates them at export time (reported as NORMALS_GENERATED). |
...
| `UV_MISSING` | info | No texture coordinates; the mesh cannot be textured. | Unwrap in a DCC, or run the generator's texture stage. |
...
| `MESH_UNINDEXED` | info | Primitive has no index buffer (~3x vertex data, no GPU vertex cache reuse). | optimize_glb welds and indexes. |
| `MESH_UNWELDED` | info | A large share of vertices are exact duplicates across all attributes. | optimize_glb welds them. |
| `SCALE_TOO_SMALL` | warning | Largest world-space dimension is below the small-scale threshold (default 0.01 m); the asset is coin-sized or was exported in the wrong unit. | Bake a uniform scale (x100 for centimetre exports) or set metersPerUnit (USD). |
| `SCALE_TOO_LARGE` | warning | Largest world-space dimension is above the large-scale threshold (default 20 m); too big for AR placement. | Bake a uniform scale down; AR assets are usually 0.1–3 m. |
| `PIVOT_NOT_AT_BASE` | info | The asset origin is not at the bottom of its bounding box; AR placement puts the origin on the surface, so the asset floats or sinks. | Translate the root so min.y (or min.z for Z-up) is 0. |
...
| `MESH_NO_MATERIAL` | warning | A mesh has no material binding; viewers show a default white/grey (or magenta) surface. | Bind a material; export_usdz binds a neutral default and reports DEFAULT_MATERIAL_BOUND. |
...
| `MATERIAL_ALPHA_BLEND` | info | Material uses alpha blending, which disables depth-write and causes sorting artifacts. | Use MASK (cutout) when the alpha is binary. |
| `MATERIAL_BLEND_WITHOUT_ALPHA` | warning | Material is set to BLEND but its base color image has no alpha channel — the blending buys artifacts for nothing. | Switch alphaMode to OPAQUE. |
| `MATERIALS_DUPLICATE` | info | Materials with identical render state under different names (one draw call each). | optimize_glb deduplicates and joins. |
...
| `TRIANGLE_BUDGET_EXCEEDED` | error | Total triangles exceed the profile limit. | optimize_glb with targetTriangles at or below the limit. |
| `VERTEX_BUDGET_EXCEEDED` | error | Total vertices exceed the profile limit. | optimize_glb (weld + simplify). |
| `DRAW_CALL_BUDGET_EXCEEDED` | error | Estimated draw calls (one per primitive / material binding) exceed the profile limit. | optimize_glb joins primitives that share a material; palette or atlas materials first. |
| `FILE_SIZE_BUDGET_EXCEEDED` | error | File size exceeds the profile limit. | optimize_glb (meshopt + WebP) typically cuts 80–95%. |
| `GPU_MEMORY_BUDGET_EXCEEDED` | error | Estimated GPU memory (decoded textures + geometry) exceeds the profile limit. | Reduce texture size or use KTX2 (textureFormat=ktx2). |
| `TEXTURE_SIZE_BUDGET_EXCEEDED` | error | A texture is larger than the profile's maximum dimension. | optimize_glb resizes textures to the profile cap. |
| `TEXTURE_BYTES_BUDGET_EXCEEDED` | error | Total encoded texture payload exceeds the profile limit. | optimize_glb re-encodes textures (WebP) and resizes. |
...
| `FIDELITY_BELOW_FLOOR` | error | Measured visual fidelity (SSIM of fixed-camera renders before vs after) is below the profile floor — the change is visible. | Raise targetTriangles or loosen the profile; inspect the comparison sheet for where it lost detail. |
...
| `ANIMATED_ASSET` | info | The asset has skins or animation clips; optimization is bone-aware and USDZ export carries the skeleton and first clip. | Budget triangles with deformation cost in mind. |
| `GENERATOR_FINGERPRINT` | info | The producing generator was recognized from the file's structure; suggestions are tailored to its known weak spots. | Nothing to do. |
