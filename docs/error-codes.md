...
| `INTENT_UP_AXIS` | `intent/up-axis` | warning | The expected up axis differs from the file's. On glTF (Y-up by definition) this is informational: the expectation names the authoring space. | If the asset lies on its side, bake a −90° X rotation; USD: set upAxis to match the geometry. |
| `INTENT_UNITS` | `intent/units` | error | Metres were expected but the USD layer declares another metersPerUnit. | Bake the scale and set metersPerUnit = 1. |
| `INTENT_SHELLS` | `intent/shells` | error | The connected shell count (welded space) is not the expected count or range. | Union the parts that should be one solid, delete loose fragments, or separate the parts. |
| `INTENT_WATERTIGHT` | `intent/watertight` | error | The asset was expected to be a closed solid (or an open surface) and is not. | Fix the topo/open-edges and topo/non-manifold findings, then re-inspect. |
| `INTENT_SIZE` | `intent/size` | error | The measured extent is outside the expected range in metres. | Scale the asset, or fix the exporter's unit scale when the cause is a unit mix-up. |
...
| `DIFF_ORIGIN_MOVED` | `diff/origin-moved` | warning | The geometry moved relative to the origin: the bounds centre shifted beyond tolerance (warning), or the landmark was reclassified with the geometry in place (info). | Translate back if unintended, or set the origin deliberately. |
...
| `OUTPUT_NOT_WRITABLE` |  | error | The `out` path cannot be written: it is a directory, its parent is a file, or the directory is not writable. Checked before any work runs, so nothing was computed. | Pass `out` as a full file path under a writable directory; missing directories are created for you. |
