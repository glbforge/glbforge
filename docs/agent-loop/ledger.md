...
## Pass 3 — 2026-09-22 — scheduled run, base `80da0f1`
...
### L2 · `open` (claimed) · see pass 1 for the full writeup

Two competing PRs open (`#11`, `#12`). Left alone this pass; anyone
resolving it should close as `fixed` with whichever PR merges, or `wontfix`
if the maintainer picks option 3 from pass 1's list.

### L4 · `open` (maintainer's call) · see pass 1 for the full writeup

`site/llms.txt` still claims "the 0.9.0 line" against five 0.8.0 packages.
Unchanged this pass; a release decision, not a docs edit.
