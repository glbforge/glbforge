...
- **Visible loss is measured.** The `minSsim` floor is the weakest of four
  ...
  Gaussian window). Floors were calibrated on the Meshy 7 fixture (1.99M
  triangles): the budget pass to 150k measures 0.958; a 40k version 0.896 with
  visibly merged hair; 10k measures 0.73.
...
### mobile-hero@1 — single hero asset on a mobile landing page (4G, mid-range GPU)
...
| minSsim | 0.94 | Calibrated on the Meshy 7 fixture: budget pass 0.958, 40k-triangle version 0.896 with visibly merged hair. The floor sits between them. |

### desktop-hero@1 — hero asset on a desktop-first marketing page
...
| minSsim | 0.96 | Viewed larger, so stricter than mobile; budget-level simplification on our fixtures still clears it. |

### product-configurator@1 — interactive product viewer; many assets coexist
...
| minSsim | 0.95 | Close-up viewing argues strict, coexistence argues lenient; the midpoint, and it clears budget-level simplification on our fixtures. |
