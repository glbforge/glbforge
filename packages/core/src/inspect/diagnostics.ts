...
export const ERROR_CODES = {
  ...
  SCALE_TOO_SMALL: spec('warning', 'Largest world-space dimension is below the small-scale threshold (default 0.01 m); the asset is coin-sized or was exported in the wrong unit.', 'Bake a uniform scale (x100 for centimetre exports) or set metersPerUnit (USD).'),
  SCALE_TOO_LARGE: spec('warning', 'Largest world-space dimension is above the large-scale threshold (default 20 m); too big for AR placement.', 'Bake a uniform scale down; AR assets are usually 0.1–3 m.'),
  PIVOT_NOT_AT_BASE: spec('info', 'The asset origin is not at the bottom of its bounding box; AR placement puts the origin on the surface, so the asset floats or sinks.', 'Translate the root so min.y (or min.z for Z-up) is 0.'),
